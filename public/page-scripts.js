...
function calcTakeHome(){
  ...
  document.getElementById('th-results-body').innerHTML=`
    ...
    <div class="result-cta"><p>Want to reduce your tax bill? We find avg. Â£4,200 in savings.</p><a href="#contact" class="btn btn-gold" style="font-size:.85rem;padding:11px 24px">Get Free Tax Review</a></div>`;
}
...
function calcVAT(){
  ...
  let html='';
  if(mustRegister){
    html=`<div class="result-row highlight" style="background:var(--red-light)"><span class="rr-label">â ï¸ VAT Registration</span><span class="rr-value" style="color:var(--red)">REQUIRED</span></div>
    <div class="result-row"><span class="rr-label">Your turnover</span><span class="rr-value">Â£${turnover.toLocaleString()}</span></div>
    <div class="result-row"><span class="rr-label">VAT threshold</span><span class="rr-value">Â£${threshold.toLocaleString()}</span></div>
    <div class="result-row"><span class="rr-label">Over threshold by</span><span class="rr-value" style="color:var(--red)">Â£${(turnover-threshold).toLocaleString()}</span></div>
    <div class="result-row"><span class="rr-label">Standard Scheme VAT due (est.)</span><span class="rr-value">Â£${Math.round(standardVAT).toLocaleString()}/yr</span></div>
    <div class="result-row"><span class="rr-label">Flat Rate Scheme VAT (est.)</span><span class="rr-value">Â£${Math.round(flatRate).toLocaleString()}/yr</span></div>
    ...
    html=`<div class="result-row highlight" style="background:rgba(200,150,62,.1)"><span class="rr-label">â³ VAT Registration</span><span class="rr-value" style="color:var(--gold)">APPROACHING</span></div>
    <div class="result-row"><span class="rr-label">Your turnover</span><span class="rr-value">Â£${turnover.toLocaleString()}</span></div>
    <div class="result-row"><span class="rr-label">Headroom remaining</span><span class="rr-value" style="color:var(--gold)">Â£${(threshold-turnover).toLocaleString()}</span></div>
    ...
    html=`<div class="result-row highlight"><span class="rr-label">â VAT Registration</span><span class="rr-value">NOT REQUIRED</span></div>
    <div class="result-row"><span class="rr-label">Your turnover</span><span class="rr-value">Â£${turnover.toLocaleString()}</span></div>
    <div class="result-row"><span class="rr-label">Headroom remaining</span><span class="rr-value" style="color:var(--green)">Â£${(threshold-turnover).toLocaleString()}</span></div>`;
  }
...
function calcDividend(){
  ...
  document.getElementById('div-results-body').innerHTML=`
    <div class="result-row" style="background:var(--gold-pale);margin:0 -36px;padding:14px 36px"><span class="rr-label">ð Optimal Strategy</span><span class="rr-value" style="color:var(--gold)">Low Salary + Dividends</span></div>
    ...
    ${saving>0?`<div class="result-row highlight"><span class="rr-label">ð° You save vs all-salary</span><span class="rr-value">${fmt(saving)}/yr</span></div>`:''}
...
// === CALCY AI CHATBOT â Full System ===
// Audio
let audioCtx=null;
...
function showReceipt(label,amount){
  ...
  inner.innerHTML=`<div class="receipt-line"><span>${label}</span><span>Â£${formatNum(amount)}</span></div>`;
...
document.getElementById('calcyArea').addEventListener('click',e=>{
  ...
  playClick();setExpression('wave',1200);moodBadge.textContent='ð HI!';spawnEmoji('ð§®');
  setTimeout(()=>moodBadge.textContent='ð§® READY',1200);
});
...
document.querySelectorAll('#calcButtons .calc-btn').forEach(btn=>{
  btn.addEventListener('click',()=>{
    ...
    if(val>='0'&&val<='9'||val==='.'){
      ...
      updateDisplay('Â£'+calcCurrent);
    }else if(val==='C'){playClick(800);calcCurrent='0';calcPrev=null;calcOp=null;updateDisplay('Â£0')
    }else if(val==='='){
      playEquals();
      if(calcPrev!==null&&calcOp){
        let r;const a=parseFloat(calcPrev),b=parseFloat(calcCurrent);
        switch(calcOp){case'+':r=a+b;break;case'-':case'â':r=a-b;break;case'Ã':r=a*b;break;case'Ã·':r=b===0?'ERR':a/b;break;default:r=b}
        if(r==='ERR'){playError();setExpression('error',1200);updateDisplay('ERR');moodBadge.textContent='â ERROR';setTimeout(()=>{updateDisplay('Â£0');moodBadge.textContent='ð§® READY'},1500)}
        else{updateDisplay('Â£'+formatNum(r));setExpression('happy',1200);moodBadge.textContent='â SOLVED';spawnText('Â£'+formatNum(r),'#2E7D5B');setTimeout(()=>moodBadge.textContent='ð§® READY',2000)}
...
const taxQA = [
  {
    ...
    emoji:"ð",display:"31 JAN",
    a: `<strong>ð Self Assessment Deadlines (2024/25 Tax Year):</strong><br><br>
â¢ <strong>5 Oct 2025</strong> â Register if you're new<br>
â¢ <strong>31 Oct 2025</strong> â Paper return deadline<br>
â¢ <strong>31 Jan 2026</strong> â Online filing & payment<br>
â¢ <strong>31 Jul 2026</strong> â 2nd payment on account<br><br>
â ï¸ Late filing = <strong>Â£100 penalty</strong> immediately, rising to Â£10/day after 3 months.<br><br>
ð¡ <em>Makesworth files returns early to avoid the January rush.</em><br><br>ð <a href="tel:02079938850" style="color:var(--gold)"><strong>020 7993 8850</strong></a> Â· ð§ <a href="mailto:info@makesworth.co.uk" style="color:var(--gold)"><strong>info@makesworth.co.uk</strong></a><br>Or <a href="#contact" style="color:var(--gold)" onclick="toggleChat()"><strong>book a free consultation</strong></a>.`
  },
  {
    q: "What is the personal allowance?",
    keywords: ["personal allowance","tax free","tax-free","12570","allowance","how much can i earn"],
    emoji:"ð·",display:"Â£12,570",
    a: `<strong>ð· Personal Allowance 2025/26: Â£12,570</strong><br><br>
â¢ Earn under <strong>Â£12,570</strong> â No income tax<br>
â¢ Earn <strong>Â£100,000+</strong> â Allowance reduces by Â£1 per Â£2 over Â£100k<br>
â¢ Earn <strong>Â£125,140+</strong> â Allowance gone completely<br>
â¢ <strong>Marriage Allowance</strong> â Transfer up to Â£1,260 to a spouse (save up to Â£252/yr)<br><br>
ð Frozen until <strong>2030/31</strong> â fiscal drag means more people pay higher rates each year.<br><br>
ð¡ <em>We help clients structure income to maximise allowances.</em><br><br>ð <a href="tel:02079938850" style="color:var(--gold)"><strong>020 7993 8850</strong></a> Â· ð§ <a href="mailto:info@makesworth.co.uk" style="color:var(--gold)"><strong>info@makesworth.co.uk</strong></a><br>Or <a href="#contact" style="color:var(--gold)" onclick="toggleChat()"><strong>book a free consultation</strong></a>.`
  },
  {
    q: "Sole trader vs limited company?",
    keywords: ["sole trader","limited company","ltd","incorporate","structure","which is better","sole vs"],
    emoji:"âï¸",display:"LTD?",
    a: `<strong>âï¸ Sole Trader vs Limited Company (2025/26):</strong><br><br>
<strong>Sole Trader:</strong> Income Tax (20-45%) + Class 2 & 4 NI on all profits. Simple setup, unlimited liability.<br><br>
<strong>Limited Company:</strong> Corporation Tax (19-25%). Take salary Â£12,570 + dividends (8.75%). Limited liability, more admin.<br><br>
ð° Earning Â£60k â Ltd saves <strong>Â£3,000âÂ£5,000/yr</strong> typically.<br><br>
ð Try our <a href="#tools" style="color:var(--gold)" onclick="toggleChat()">free comparison tool</a>!<br>
ð¡ <em>We handle incorporations every week.</em><br><br>ð <a href="tel:02079938850" style="color:var(--gold)"><strong>020 7993 8850</strong></a> Â· ð§ <a href="mailto:info@makesworth.co.uk" style="color:var(--gold)"><strong>info@makesworth.co.uk</strong></a><br>Or <a href="#contact" style="color:var(--gold)" onclick="toggleChat()"><strong>book a free consultation</strong></a>.`
  },
  {
    q: "Do I need to register for VAT?",
    keywords: ["vat","register","threshold","90000","vat registration","turnover","flat rate"],
    emoji:"ð",display:"Â£90K",
    a: `<strong>ð VAT Registration (2025/26):</strong><br><br>
â¢ <strong>Mandatory: Â£90,000</strong> â Must register if turnover exceeds this in any 12-month period<br>
â¢ <strong>Deregistration: Â£88,000</strong><br><br>
<strong>Schemes:</strong> Standard (20% VAT, reclaim on expenses) Â· Flat Rate (fixed % of turnover) Â· Cash Accounting<br><br>
â ï¸ You can register <strong>voluntarily</strong> below Â£90k if clients are VAT-registered.<br><br>
ð Try our <a href="#tools" style="color:var(--gold)" onclick="toggleChat()">free VAT checker</a>!<br>
ð¡ <em>We help clients choose the best VAT scheme.</em><br><br>ð <a href="tel:02079938850" style="color:var(--gold)"><strong>020 7993 8850</strong></a> Â· ð§ <a href="mailto:info@makesworth.co.uk" style="color:var(--gold)"><strong>info@makesworth.co.uk</strong></a><br>Or <a href="#contact" style="color:var(--gold)" onclick="toggleChat()"><strong>book a free consultation</strong></a>.`
  },
  {
    ...
    emoji:"ð°",display:"20/40/45",
    a: `<strong>ð° Income Tax Rates 2025/26 (England, Wales & NI):</strong><br><br>
...
<tr style="background:#f8f9fa"><td style="padding:5px 6px">Personal Allowance</td><td style="padding:5px 6px">Up to Â£12,570</td><td style="padding:5px 6px"><strong>0%</strong></td></tr>
<tr><td style="padding:5px 6px">Basic</td><td style="padding:5px 6px">Â£12,571âÂ£50,270</td><td style="padding:5px 6px"><strong>20%</strong></td></tr>
<tr style="background:#f8f9fa"><td style="padding:5px 6px">Higher</td><td style="padding:5px 6px">Â£50,271âÂ£125,140</td><td style="padding:5px 6px"><strong>40%</strong></td></tr>
<tr><td style="padding:5px 6px">Additional</td><td style="padding:5px 6px">Over Â£125,140</td><td style="padding:5px 6px"><strong>45%</strong></td></tr>
</table><br>
ð Frozen until <strong>2030/31</strong>.<br>
ð¡ <em>Use our <a href="#tools" style="color:var(--gold)" onclick="toggleChat()">Take-Home Pay calculator</a> to see what you keep.</em><br><br>ð <a href="tel:02079938850" style="color:var(--gold)"><strong>020 7993 8850</strong></a> Â· ð§ <a href="mailto:info@makesworth.co.uk" style="color:var(--gold)"><strong>info@makesworth.co.uk</strong></a><br>Or <a href="#contact" style="color:var(--gold)" onclick="toggleChat()"><strong>book a free consultation</strong></a>.`
  },
  {
    q: "How much is the dividend allowance?",
    keywords: ["dividend","allowance","dividend tax","8.75","33.75","company director","pay myself"],
    emoji:"ð§¾",display:"Â£500",
    a: `<strong>ð§¾ Dividend Tax 2025/26:</strong><br><br>
â¢ <strong>Tax-free allowance: Â£500</strong><br>
â¢ Basic rate: <strong>8.75%</strong> Â· Higher: <strong>33.75%</strong> Â· Additional: <strong>39.35%</strong><br><br>
â ï¸ <strong>From April 2026</strong>: Basic â 10.75%, Higher â 35.75% (+2% each)<br><br>
<strong>Optimal:</strong> Salary Â£12,570 + dividends = <strong>Â£3,000âÂ£8,000/yr cheaper</strong> than all salary.<br><br>
ð Try our <a href="#tools" style="color:var(--gold)" onclick="toggleChat()">Dividend vs Salary optimiser</a>!<br>
ð¡ <em>We plan the most tax-efficient salary/dividend mix for every client.</em><br><br>ð <a href="tel:02079938850" style="color:var(--gold)"><strong>020 7993 8850</strong></a> Â· ð§ <a href="mailto:info@makesworth.co.uk" style="color:var(--gold)"><strong>info@makesworth.co.uk</strong></a><br>Or <a href="#contact" style="color:var(--gold)" onclick="toggleChat()"><strong>book a free consultation</strong></a>.`
  },
  {
    q: "Can I claim working from home?",
    keywords: ["working from home","wfh","home office","claim","expenses","remote","work from home"],
    emoji:"ð ",display:"Â£6/WK",
    a: `<strong>ð  Working From Home Tax Relief (2025/26):</strong><br><br>
<strong>Employed:</strong> Â£6/week flat rate (Â£312/yr) no receipts needed, or claim actual costs. Employer must <strong>require</strong> WFH.<br><br>
<strong>Self-employed:</strong> Simplified expenses: Â£10/month (25-50 hrs), Â£18 (51-100), Â£26 (101+). Or actual proportion of bills.<br><br>
â ï¸ Exclusive business use of a room may affect CGT exemption on your home.<br><br>
ð¡ <em>Many clients miss this deduction â we'll check what you can claim.</em><br><br>ð <a href="tel:02079938850" style="color:var(--gold)"><strong>020 7993 8850</strong></a> Â· ð§ <a href="mailto:info@makesworth.co.uk" style="color:var(--gold)"><strong>info@makesworth.co.uk</strong></a><br>Or <a href="#contact" style="color:var(--gold)" onclick="toggleChat()"><strong>book a free consultation</strong></a>.`
  },
  {
    q: "What is Making Tax Digital?",
    keywords: ["making tax digital","mtd","digital","hmrc","software","quarterly","record keeping"],
    emoji:"ð±",display:"MTD",
    a: `<strong>ð± Making Tax Digital (MTD):</strong><br><br>
<strong>VAT</strong> (live): All VAT businesses must keep digital records + file via software.<br><br>
<strong>Income Tax</strong> â rolling out:<br>
â¢ <strong>April 2026</strong>: Earning over Â£50,000<br>
â¢ <strong>April 2027</strong>: Over Â£30,000<br><br>
Means quarterly updates to HMRC via Xero/QuickBooks + end-of-period statement.<br><br>
ð¡ <em>Makesworth is fully MTD-ready with Xero & Dext.</em><br><br>ð <a href="tel:02079938850" style="color:var(--gold)"><strong>020 7993 8850</strong></a> Â· ð§ <a href="mailto:info@makesworth.co.uk" style="color:var(--gold)"><strong>info@makesworth.co.uk</strong></a><br>Or <a href="#contact" style="color:var(--gold)" onclick="toggleChat()"><strong>book a free consultation</strong></a>.`
  },
  {
    ...
    emoji:"ð",display:"15%",
    a: `<strong>ð Employer NI 2025/26:</strong><br><br>
â¢ <strong>Rate: 15%</strong> (up from 13.8%)<br>
â¢ <strong>Threshold: Â£96/week</strong> (Â£5,000/yr) â down from Â£175/week<br><br>
<strong>Employment Allowance:</strong> Â£10,500/yr off your NI bill (if bill was under Â£100k prev year).<br><br>
<strong>Employee NI:</strong> 8% on Â£12,570âÂ£50,270, then 2% above.<br><br>
ð¡ <em>We help businesses structure payroll tax-efficiently.</em><br><br>ð <a href="tel:02079938850" style="color:var(--gold)"><strong>020 7993 8850</strong></a> Â· ð§ <a href="mailto:info@makesworth.co.uk" style="color:var(--gold)"><strong>info@makesworth.co.uk</strong></a><br>Or <a href="#contact" style="color:var(--gold)" onclick="toggleChat()"><strong>book a free consultation</strong></a>.`
  },
  {
    q: "How does student loan repayment work?",
    keywords: ["student loan","repayment","plan 1","plan 2","plan 4","plan 5","graduate","university"],
    emoji:"ð",display:"9%",
    a: `<strong>ð Student Loan Repayments 2025/26:</strong><br><br>
...
<tr style="background:#f8f9fa"><td style="padding:4px 5px">Plan 1</td><td style="padding:4px 5px">Â£22,015</td><td style="padding:4px 5px">9%</td></tr>
<tr><td style="padding:4px 5px">Plan 2</td><td style="padding:4px 5px">Â£27,295</td><td style="padding:4px 5px">9%</td></tr>
<tr style="background:#f8f9fa"><td style="padding:4px 5px">Plan 4</td><td style="padding:4px 5px">Â£27,660</td><td style="padding:4px 5px">9%</td></tr>
<tr><td style="padding:4px 5px">Plan 5</td><td style="padding:4px 5px">Â£25,000</td><td style="padding:4px 5px">9%</td></tr>
<tr style="background:#f8f9fa"><td style="padding:4px 5px">Postgrad</td><td style="padding:4px 5px">Â£21,000</td><td style="padding:4px 5px">6%</td></tr>
</table><br>
ð You repay 9% on earnings <strong>above</strong> the threshold only.<br>
ð¡ <em>Our <a href="#tools" style="color:var(--gold)" onclick="toggleChat()">Take-Home Pay calculator</a> includes student loan deductions!</em><br><br>ð <a href="tel:02079938850" style="color:var(--gold)"><strong>020 7993 8850</strong></a> Â· ð§ <a href="mailto:info@makesworth.co.uk" style="color:var(--gold)"><strong>info@makesworth.co.uk</strong></a><br>Or <a href="#contact" style="color:var(--gold)" onclick="toggleChat()"><strong>book a free consultation</strong></a>.`
  }
];

// Calcy expression map per question topic
const calcyReactions = [
  {expr:'thinking',sound:'click',spawn:'ð',badge:'ð DEADLINE'},
  {expr:'happy',sound:'equals',spawn:'ð·',badge:'ð· ALLOWANCE'},
  {expr:'thinking',sound:'click',spawn:'âï¸',badge:'âï¸ COMPARING'},
  {expr:'bounce',sound:'cash',spawn:'ð',badge:'ð VAT CHECK'},
  {expr:'thinking',sound:'click',spawn:'ð°',badge:'ð° TAX RATES'},
  {expr:'happy',sound:'cash',spawn:'ð§¾',badge:'ð§¾ DIVIDENDS'},
  {expr:'wave',sound:'equals',spawn:'ð ',badge:'ð  WFH'},
  {expr:'thinking',sound:'click',spawn:'ð±',badge:'ð± MTD'},
  {expr:'bounce',sound:'cash',spawn:'ð',badge:'ð EMPLOYER NI'},
  {expr:'happy',sound:'success',spawn:'ð',badge:'ð STUDENT LOAN'}
];
...
function toggleChat(){
  ...
  if(cb.classList.contains('open')){
    ...
    moodBadge.textContent='ð HELLO';spawnEmoji('ð§®');
    setTimeout(()=>moodBadge.textContent='ð§® READY',1200);
  }
}
...
function doCalcyReaction(idx){
  ...
  // Reset badge
  setTimeout(()=>moodBadge.textContent='ð§® READY',3000);
}
...
function sendChat(){
  ...
  if(bestScore>=3){
    ...
    playClick();setExpression('thinking',1500);moodBadge.textContent='ð¤ HMMMM';
    spawnText('?','#C8963E');
    setTimeout(()=>moodBadge.textContent='ð§® READY',2500);
    typeResponse(`Thanks for your question! Our team can give you a personalised answer.<br><br>ð <strong><a href="tel:02079938850" style="color:var(--gold)">020 7993 8850</a></strong><br>ð§ <strong><a href="mailto:info@makesworth.co.uk" style="color:var(--gold)">info@makesworth.co.uk</a></strong><br><br>Or <a href="#contact" style="color:var(--gold)" onclick="toggleChat()"><strong>book a free consultation</strong></a> â we respond within 4 hours.<br><br><em>Try these popular questions:</em>
    <div class="chat-suggestions" style="margin-top:8px">
      <span class="chat-sug" onclick="askQ(0)">ð Self-assessment</span>
      <span class="chat-sug" onclick="askQ(4)">ð° Tax rates</span>
      <span class="chat-sug" onclick="askQ(2)">âï¸ Sole trader vs Ltd</span>
    </div>`);
  }
}
...
const wcMessages=[
  'ð¬ Need tax help? Click me!',
  'ð§® I can save you Â£4,200!',
  'ð Self-assessment due soon!',
  'ð· Free tax advice inside!',
  'ð Hi there! Click me!',
  'ð Check our free tools!',
  'ð¯ Sole trader or Ltd?',
  'ð° Know your tax rates?'
];
let wcMsgIdx=0;
let wcPaused=false;
let wcTimeout=null;
